"""
AI EXCEL AUTOMATION ENGINE v3.0
...
from utils.analyzer import (basic_analysis, correlation_analysis, trend_analysis,
    compute_kpis, pareto_analysis, moving_average, compare_groups)
...
from utils.profiler import (profile_dataset, profile_columns,
    detect_constant_columns, detect_high_cardinality, get_correlation_pairs)
...
def process_nlq(df, query):
    ...
    try:
        if 'show columns' in q or 'list columns' in q:
            return {'success':True,'explanation':f'{len(df.columns)} columns:','dataframe':pd.DataFrame({'Column':df.columns,'Type':[str(d) for d in df.dtypes]}),'value':None}
        ...
        if m:
            ...
            if gc and vc:
                ...
                return {'success':True,'explanation':f'{fn.title()} of "{vc}" by "{gc}":','dataframe':g,'value':None}
            ...
            if vc:
                for cc in df.select_dtypes(['object','category']).columns:
                    ...
                    if g1.lower() in vals.values and g2.lower() in vals.values:
                        comp = pd.DataFrame({g1:df[vals==g1.lower()][vc].describe(),g2:df[vals==g2.lower()][vc].describe()})
                        return {'success':True,'explanation':f'Comparing "{vc}":','dataframe':comp,'value':None}
        ...
        if m:
            ...
            if col and pd.api.types.is_numeric_dtype(df[col]):
                fm = {'average':'mean','avg':'mean','mean':'mean','sum':'sum','total':'sum','min':'min','max':'max','median':'median','count':'count'}
                val = getattr(df[col],fm.get(fs,'mean'))()
                return {'success':True,'explanation':f'{fm.get(fs,"mean").title()} of "{col}":','dataframe':None,'value':f'{val:,.2f}'}
        m = re.search(r'correlat\w*\s+(?:with\s+)?(\w+)', q)
        if m:
            col = _find_column(df,m.group(1))
            if col and pd.api.types.is_numeric_dtype(df[col]):
                cr = df.select_dtypes('number').corr()[col].drop(col).sort_values(key=abs,ascending=False).reset_index()
                cr.columns = ['Column','Correlation']
                return {'success':True,'explanation':f'Correlations with "{col}":','dataframe':cr,'value':None}
        m = re.search(r'filter\s+(?:where\s+)?(\w+)\s+(?:is|equals?|==|contains?)\s+(.+)', q)
        if m:
            col,val = _find_column(df,m.group(1)),m.group(2).strip().strip('"\'')
            if col:
                if pd.api.types.is_numeric_dtype(df[col]):
                    try: f = df[df[col]==float(val)]
                    except: f = df[df[col].astype(str).str.contains(val,case=False,na=False)]
                else: f = df[df[col].astype(str).str.contains(val,case=False,na=False)]
                return {'success':True,'explanation':f'{len(f)} rows matched:','dataframe':f,'value':None}
        ...
        if m:
            ...
            if col and pd.api.types.is_numeric_dtype(df[col]):
                ...
                return {'success':True,'explanation':f'{len(f)} rows:','dataframe':f,'value':None}
        m = re.search(r'sort\s+(?:by\s+)?(\w+)\s*(asc|desc)?', q)
        if m:
            col = _find_column(df,m.group(1))
            if col:
                asc = (m.group(2) or 'desc').startswith('asc')
                return {'success':True,'explanation':f'Sorted by "{col}":','dataframe':df.sort_values(col,ascending=asc),'value':None}
        m = re.search(r'unique\s+(\w+)', q)
        if m:
            col = _find_column(df,m.group(1))
            if col:
                vc = df[col].value_counts().reset_index(); vc.columns=[col,'Count']
                return {'success':True,'explanation':f'{df[col].nunique()} unique values:','dataframe':vc,'value':None}
        m = re.search(r'describe\s+(\w+)', q)
        if m:
            col = _find_column(df,m.group(1))
            if col: return {'success':True,'explanation':f'Description of "{col}":','dataframe':df[col].describe().reset_index(),'value':None}
        return {'success':False,'explanation':'Try: "top 5 by sales", "average of profit", "group by region and sum sales", "compare sales between North and South", "filter where sales between 100 and 500", "what correlates with profit?"','dataframe':None,'value':None}
    except Exception as e:
        return {'success':False,'explanation':f'Error: {e}','dataframe':None,'value':None}
...
with st.sidebar:
    st.markdown("""<div style="text-align:center;padding:0.8rem 0"><span style="font-size:2.5rem">🚀</span>
        ...
        <p style="color:#94A3B8;font-size:0.8rem">Premium Data Automation v3.0</p></div>""", unsafe_allow_html=True)
    st.markdown("---")
...
if demo_path and os.path.exists(demo_path): st.session_state['demo_file'] = demo_path

# Smart Join - second file upload
st.markdown('<div class="section-header">🔗 Smart Join (Optional — Upload Second File)</div>', unsafe_allow_html=True)
second_file = st.file_uploader("Upload a second file to join/merge",type=['xlsx','xls','csv'],key='file2',help="Auto-detects matching columns for VLOOKUP-style join")
...
if active_file is not None:
    if uploaded_file: st.session_state.pop('demo_file',None)
    ...
    st.markdown(f'<div style="margin:0.8rem 0"><span class="badge badge-purple">📁 {fn}</span><span class="badge badge-blue">📐 {df_raw.shape[0]} rows × {df_raw.shape[1]} cols</span><span class="badge badge-green">💾 {result.get("file_size_kb","?")} KB</span></div>', unsafe_allow_html=True)

    if show_raw:
        with st.expander("📋 Raw Data",expanded=False): st.dataframe(df_raw,use_container_width=True,height=300)

    # CLEAN
    st.markdown('<div class="section-header">🧹 Data Cleaning Engine</div>', unsafe_allow_html=True)
    with st.spinner("Cleaning..."): df_clean, cr_report = clean_data(df_raw)

    cc = st.columns(4)
    total_missing_fixed = sum(cr_report.get('missing_values_before',{}).values()) + sum(cr_report.get('markers_replaced',{}).values())
    items = [(cr_report["duplicates_removed"],"Duplicates Removed"),(total_missing_fixed,"Missing Fixed"),
             (len(cr_report.get('type_conversions',{})),"Type Conversions"),(f"{cr_report.get('data_completeness',100)}%","Completeness")]
    for i,(v,l) in enumerate(items):
        with cc[i]: st.markdown(f'<div class="kpi-card"><p class="kpi-value">{v}</p><p class="kpi-label">{l}</p></div>',unsafe_allow_html=True)

    # Show missing fixes PROMINENTLY
    ...
    if markers:
        ...
        for col,count in markers.items():
            st.markdown(f'<div class="insight-card critical"><p class="insight-category">Column: {col}</p><p class="insight-text">🔍 {count} placeholder value(s) (empty, N/A, -, null, etc.) detected and converted to missing</p></div>',unsafe_allow_html=True)
    ...
    if missing_actions:
        ...
        for col,action in missing_actions.items():
            st.markdown(f'<div class="insight-card warning"><p class="insight-category">Column: {col}</p><p class="insight-text">🔧 {action}</p></div>',unsafe_allow_html=True)
    elif not markers:
        st.markdown('<div style="background:rgba(16,185,129,0.1);border:1px solid rgba(16,185,129,0.3);border-radius:12px;padding:0.8rem;margin:0.8rem 0"><p style="color:#34D399;font-size:0.85rem;margin:0">✅ No missing values — dataset is complete!</p></div>',unsafe_allow_html=True)
    ...
    with st.expander("📋 Cleaned Data Preview"): st.dataframe(df_clean,use_container_width=True,height=300)
    ...
    pc = st.columns(4)
    for i,(v,l) in enumerate([(f"{ds_profile['rows']:,}","Rows"),(f"{ds_profile['columns']}","Columns"),(f"{ds_profile['missing_pct']}%","Missing"),(ds_profile['memory_usage'],"Memory")]):
        with pc[i]: st.markdown(f'<div class="kpi-card"><p class="kpi-value">{v}</p><p class="kpi-label">{l}</p></div>',unsafe_allow_html=True)
    ...
    with st.expander("📊 Column Profiles"):
        ...
        for p in col_profiles:
            ...
            if p.get('category')=='numeric': r.update({'Mean':p.get('mean',''),'Std':p.get('std',''),'Min':p.get('min',''),'Max':p.get('max','')})
            elif p.get('category')=='categorical': r.update({'Top Value':p.get('most_common',''),'Top%':p.get('most_common_pct','')})
            pd_rows.append(r)
    ...
    if kpis:
        ...
        for i,cn in enumerate(kn[:3]):
            with r1[i]:
                ...
                st.markdown(f'<div class="kpi-card"><p class="kpi-value">{k["total"]:,.0f}</p><p class="kpi-label">Total {cn}</p><p class="kpi-delta {dc}">{di} {k["growth_pct"]:+.1f}%</p></div>',unsafe_allow_html=True)
        if len(kn)>3:
        ...
        with tabs[1]:
            if date_cols:
                ...
                try:
                    ...
                    if tr:
                        st.markdown(f'<div style="background:#1E293B;border-radius:10px;padding:0.8rem;margin-bottom:0.8rem"><span class="badge badge-purple">Trend: {tr["direction"]}</span><span class="badge badge-blue">Change: {tr["change_pct"]:+.1f}%</span></div>',unsafe_allow_html=True)
    ...
    insights = generate_insights(df_clean, cr_report)
    narrative = generate_summary_narrative(df_clean, kpis, insights)
    st.markdown(f'<div style="background:linear-gradient(135deg,rgba(124,58,237,0.1),rgba(59,130,246,0.1));border:1px solid rgba(124,58,237,0.2);border-radius:12px;padding:1rem;margin-bottom:1rem"><p style="color:#A78BFA;font-size:0.7rem;text-transform:uppercase;letter-spacing:1.5px;margin:0 0 0.4rem">Executive Summary</p><p style="color:#E2E8F0;font-size:0.9rem;line-height:1.5;margin:0">{narrative}</p></div>',unsafe_allow_html=True)
    for ins in insights:
        st.markdown(f'<div class="insight-card {ins["severity"]}"><p class="insight-category">{ins["category"]}</p><p class="insight-text">{ins["icon"]} {ins["insight"]}</p></div>',unsafe_allow_html=True)
    ...
    ml_result = None; anomaly_results = {}; anom_summaries = []
    if is_ml and SKLEARN_AVAILABLE:
        ml_m = 'isolation_forest' if 'Forest' in anomaly_method else 'lof'
        cont = max(0.01,min(0.3,1.0/anomaly_sensitivity))
        with st.spinner(f"Running {anomaly_method}..."): ml_result = detect_ml_anomalies(df_clean,method=ml_m,contamination=cont)
        ...
        with st.spinner("Statistical scan..."): anomaly_results = detect_all_anomalies(df_clean,'zscore',anomaly_sensitivity); anom_summaries = anomaly_summary(anomaly_results)
    else:
        m = 'zscore' if 'Z-Score' in anomaly_method else 'iqr'
        with st.spinner("Scanning..."): anomaly_results = detect_all_anomalies(df_clean,m,anomaly_sensitivity); anom_summaries = anomaly_summary(anomaly_results)
    ...
    if fin_cols and len(fin_cols) >= 1:
        ...
        with ftabs[2]:
            if fin_cols:
                ...
                audit = tax_vat_audit(df_clean,tamount,ttax if ttax!='None' else None,item_col=item_col if item_col!='None' else None)
                if 'error' not in audit:
                    for f in audit.get('findings',[]):
                        sev_class = {'critical':'critical','warning':'warning','positive':'positive'}.get(f['severity'],'info')
                        st.markdown(f'<div class="insight-card {sev_class}"><p class="insight-category">{f["severity"].upper()}</p><p class="insight-text">{f["icon"]} {f["issue"]}</p></div>',unsafe_allow_html=True)
                        if f.get('details'): st.markdown(f"  *{f['details']}*")
            else: st.info("No financial columns detected.")
        with ftabs[3]:
            if fin_cols:
                ratios = financial_ratios(df_clean,revenue_col=fin_cols[0] if len(fin_cols)>=1 else None,cost_col=fin_cols[1] if len(fin_cols)>=2 else None,profit_col=fin_cols[2] if len(fin_cols)>=3 else None)
                if ratios:
            ...
            try:
                ...
                if matches:
                    ...
                    if st.button("🔗 Execute Join",use_container_width=True):
                        jresult = smart_join(df_clean,df2,left_key,right_key,join_type)
                        if 'error' not in jresult:
                            ...
                            buf.seek(0)
                            st.download_button("📥 Download Merged Data",buf,"merged_data.xlsx","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",use_container_width=True)
    ...
    st.markdown('<div style="background:#1E293B;border-radius:10px;padding:0.8rem;margin-bottom:0.8rem"><p style="color:#94A3B8;font-size:0.8rem;margin:0"><b style="color:#A78BFA">Try:</b> "top 5 by sales" · "average of profit" · "group by region and sum sales" · "compare sales between North and South" · "what correlates with profit?"</p></div>',unsafe_allow_html=True)
    ...
    rc = st.columns(2)
    with rc[0]:
        st.markdown('<div class="kpi-card"><p style="font-size:1.8rem;margin:0">📊</p><p class="kpi-label" style="margin:0.3rem 0">Excel Report (Formula-Rich)</p><p style="color:#64748B;font-size:0.7rem;margin:0">Live SUM/AVG/MIN/MAX formulas + conditional formatting</p></div>',unsafe_allow_html=True)
        try:
            ...
            st.download_button("📥 Download Excel Report",ebuf,f"ai_report_{sn}.xlsx","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",use_container_width=True,key='dl_xl')
        except Exception as e: st.error(f"Excel error: {e}")
    with rc[1]:
        st.markdown(f'<div class="kpi-card"><p style="font-size:1.8rem;margin:0">📄</p><p class="kpi-label" style="margin:0.3rem 0">PDF Report {"(White-Label)" if brand_name else ""}</p><p style="color:#64748B;font-size:0.7rem;margin:0">{"Branded for "+brand_name if brand_name else "Professional report with TOC & insights"}</p></div>',unsafe_allow_html=True)
        try:
            ml_info = ml_anomaly_summary(ml_result) if ml_result else None
            pbuf = PDFReportGenerator.generate(df_clean,kpis,insights,anom_summaries,narrative,ds_profile,ml_info,cr_report,brand_name=brand_name or None,brand_color_hex=brand_color,logo_path=logo_path)
            sn = fn.rsplit('.',1)[0] if '.' in fn else fn
    ...
    st.markdown("---")
    st.markdown('<div style="text-align:center;padding:0.8rem"><p style="color:#64748B;font-size:0.75rem">🚀 <b>AI Excel Automation Engine v3.0</b> — Built by <a href="https://github.com/muh0210" style="color:#7C3AED">Muhammad Rajput</a><br><span style="font-size:0.65rem">Streamlit · Pandas · Plotly · scikit-learn · Python</span></p></div>',unsafe_allow_html=True)

else:
    fc = st.columns(3)
    features = [("🧹","Smart Cleaning","Detects empty cells, N/A, spaces as missing — fills intelligently"),
    ...
    for i,(icon,title,desc) in enumerate(features):
        with fc[i%3]:
            st.markdown(f'<div class="kpi-card" style="margin-bottom:0.8rem;text-align:left;padding:1.2rem"><p style="font-size:1.8rem;margin:0">{icon}</p><p style="color:#E2E8F0;font-size:0.95rem;font-weight:700;margin:0.4rem 0 0.2rem">{title}</p><p style="color:#94A3B8;font-size:0.75rem;margin:0;line-height:1.3">{desc}</p></div>',unsafe_allow_html=True)
    st.markdown('<div style="text-align:center;margin-top:1.5rem;padding:1.5rem;background:linear-gradient(135deg,rgba(124,58,237,0.08),rgba(59,130,246,0.08));border-radius:16px;border:1px dashed rgba(124,58,237,0.3)"><p style="font-size:2.2rem;margin:0">📂</p><p style="color:#A78BFA;font-size:1rem;font-weight:600;margin:0.4rem 0">Upload a file or try a demo dataset</p><p style="color:#64748B;font-size:0.8rem">Supports Excel (.xlsx, .xls) and CSV up to 50MB</p></div>',unsafe_allow_html=True)
